...
class TestPreprocess(unittest.TestCase):
    def test_process_include(self) -> None:
        ...
        # No include
        line, used, problem = core.preprocess.process_include(
            "foo bar",
            deps,
            "foo",
        )
    ...
    def test_process_images(self) -> None:
        ...
        # No images
        line, used, problems = core.preprocess.process_images(
    ...
    def test_check_strict_deps(self) -> None:
        # OK
        self.assertIsNone(
            core.preprocess.check_strict_deps(frozenset(["a", "b"]), frozenset(["a", "b"]), "foo")
        )

        # Used but not declared
        self.assertIsNotNone(
            core.preprocess.check_strict_deps(frozenset(["a", "b"]), frozenset(["a"]), "foo")
        )

        # Declared but not used
        self.assertIsNotNone(
            core.preprocess.check_strict_deps(frozenset(["a"]), frozenset(["a", "b"]), "foo")
        )

        # Both
        self.assertIsNotNone(
            core.preprocess.check_strict_deps(frozenset(["a", "c"]), frozenset(["a", "d"]), "foo")
        )
    ...
    def test_preprocess(self) -> None:
        ...
        problems = core.preprocess.preprocess(data, deps, images, "a")
        self.assertEqual(problems, [])
